...
TEST_RATIO=${TEST_RATIO:-0.8}
SEED=${SEED:-315}
MAX_SCENES=${MAX_SCENES:-330}
MIN_TRAIN_SCENES=${MIN_TRAIN_SCENES:-290}
MIN_READY_SCENES=${MIN_READY_SCENES:-310}

RUN_TITLE=${:-""}
SPLIT_TAG="test${TEST_RATIO}_seed${SEED}"
SPLIT_TAG=${SPLIT_TAG//./p}
SPLIT_DIR=${SPLIT_DIR:-$ROOT/splits/pose-SD-splits-${SPLIT_TAG}}

EPOCHS=${EPOCHS:-100}
OUTPUT=${OUTPUT:-checkpoints/${SPLIT_TAG}}
RUN_NAME=${RUN_NAME:-${RUN_TITLE}-${SPLIT_TAG}}
WANDB_ID=${WANDB_ID:-}

PROMPT=${PROMPT:-""}
PROMPT_TEMPLATE=${PROMPT_TEMPLATE:-""}
MAX_PAIR_DIST=${MAX_PAIR_DIST:-0}
MIN_PAIR_IOU=${MIN_PAIR_IOU:-0.3}
MIN_REF_SPACING=${MIN_REF_SPACING:-0.5}
MIN_VIEW_COS=${MIN_VIEW_COS:-0.2}
MAX_ROTATION_DEG=${MAX_ROTATION_DEG:-220.0}
MAX_FOCAL_RATIO=${MAX_FOCAL_RATIO:-9990099}
PAIR_PREFILTER_TOPK=${PAIR_PREFILTER_TOPK:-256}
CANDIDATE_POOL_TOPK=${CANDIDATE_POOL_TOPK:-64}
MAX_TRIPLETS=${MAX_TRIPLETS:-100000000}
