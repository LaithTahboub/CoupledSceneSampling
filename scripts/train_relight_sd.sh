#!/bin/bash
# RelightSD training: 3-view (ref1, ref2, target) with Plucker ray conditioning.
# Multi-GPU via torchrun (8x A6000).

#SBATCH --job-name=css-relight-sd
...
#SBATCH --output=/vulcanscratch/ltahboub/CoupledSceneSampling/logs/train_relight_sd_%j.out
#SBATCH --error=/vulcanscratch/ltahboub/CoupledSceneSampling/logs/train_relight_sd_%j.err
...
RUN_NAME=${RUN_NAME:-relight_sd_v10}
...
torchrun \
    ...
    -m css.train.train_relight_sd "${ARGS[@]}"
