...
# ---------------------------------------------------------------------------
# Scene-scale normalization for Plucker maps
# ---------------------------------------------------------------------------
...
def compute_scene_scale(
    ...
    """Compute a robust scene scale from camera positions.

    Uses the percentile of distances from the centroid of all cameras.
    This gives a reference-agnostic measure of scene extent.

    Args:
        positions: (N, 3) array of camera positions.
        percentile: which percentile of centroid-distances to use.

    Returns:
        Scene scale scalar s.  If fewer than 2 cameras, returns 0.0.
    """
    if len(positions) < 2:
        return 0.0
...
def normalize_c2w_translation(
    c2w_anchor: np.ndarray,
    c2w_other: np.ndarray,
    scene_scale: float,
    alpha: float = 2.0,
    eps: float = 1e-8,
) -> np.ndarray:
    """Normalize the translation of c2w_other relative to c2w_anchor.

    Computes the relative translation in the anchor frame, rescales it
    by alpha / (scene_scale + eps), then reconstructs the world-frame c2w.
    Rotation is unchanged.

    Args:
        c2w_anchor: (4, 4) anchor camera-to-world (defines the origin).
        c2w_other:  (4, 4) camera-to-world to normalize.
        scene_scale: scalar s from compute_scene_scale().
        alpha: target magnitude scale (default 2.0).
        eps: numerical guard.

    Returns:
        (4, 4) normalized c2w_other.  When compute_plucker_tensor later
        computes inv(c2w_anchor) @ result, the relative translation will
        be alpha * t_rel / (s + eps).
    """
    R_anchor = c2w_anchor[:3, :3]
    t_anchor = c2w_anchor[:3, 3]
    t_other = c2w_other[:3, 3]

    # Relative translation in anchor's local frame
    t_rel = R_anchor.T @ (t_other - t_anchor)
    t_norm = alpha * t_rel / (scene_scale + eps)

    # Reconstruct in world frame
    c2w_norm = c2w_other.copy()
    c2w_norm[:3, 3] = R_anchor @ t_norm + t_anchor
    return c2w_norm
