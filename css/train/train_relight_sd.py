"""Training script for RelightSD (3-view, Plucker-conditioned).
...
from css.models.EMA import EMAModel, load_relight_sd_checkpoint, save_relight_sd_checkpoint
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train RelightSD with multi-GPU support")

    # Paths
    p.add_argument("--scenes", nargs="*", default=None)
    p.add_argument("--scenes-file", type=str, default=None)
    p.add_argument("--output", type=str, default="checkpoints/relight_sd_v2")
...
def main() -> None:
    ...
    # Model
    model = RelightSD(pretrained_model=args.pretrained_model, device=str(device))
    ...
    if cnfg.resume_from:
        ...
        resumed = load_relight_sd_checkpoint(
