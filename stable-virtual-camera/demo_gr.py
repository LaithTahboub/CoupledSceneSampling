import os
os.environ.setdefault("GRADIO_TEMP_DIR", os.path.join(os.path.dirname(__file__), ".gradio_tmp"))
os.makedirs(os.environ["GRADIO_TEMP_DIR"], exist_ok=True)

import gradio_client.utils

# --- PATCH START ---
# Fix for Pydantic v2 / Gradio compatibility issues

# 1. Patch get_type (Fixes "TypeError: argument of type 'bool' is not iterable")
_original_get_type = gradio_client.utils.get_type

def _patched_get_type(schema):
    if isinstance(schema, bool):
        return "any"  # Return a dummy type for boolean schemas
    return _original_get_type(schema)

gradio_client.utils.get_type = _patched_get_type

# 2. Patch _json_schema_to_python_type (Fixes "APIInfoParseError: Cannot parse schema True")
_original_schema_converter = gradio_client.utils._json_schema_to_python_type

def _patched_schema_converter(schema, defs=None):
    if isinstance(schema, bool):
        return "any"  # Return a dummy type for boolean schemas
    return _original_schema_converter(schema, defs)

gradio_client.utils._json_schema_to_python_type = _patched_schema_converter

print("✅ PATCH APPLIED: Fixed Pydantic v2 boolean schema conflicts.")
import copy
import json
import os
